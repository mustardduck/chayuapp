...
#import "CYTuiKuangLeftCell.h"
#import "CYTuiKuanRightCell.h"
...
{
    static NSString *UserCellIdentifier = @"CYTuiKuanRightCell";
    static NSString *SellerCellIdentifier = @"SellerCell";
    NSInteger row = indexPath.row;
    if ([[arr_data[row] objectForKey:@"source"] integerValue]==1) {
         CYTuiKuanRightCell *cell = [tableView dequeueReusableCellWithIdentifier:UserCellIdentifier];
        if (!cell) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"CYTuiKuanRightCell" owner:nil options:nil] firstObject];
        }
        ...
        if ([arr count]==0) {
            cell.contentHeight_cons.constant = 37;
        }else{
            cell.contentHeight_cons.constant = (37 +32*[arr count]);
        }
        ...
        NSArray *imgArr = [dic objectForKey:@"imgs"];
        if ([imgArr count]>0) {
            cell.contentHeight_cons.constant = 210;
        }
        for (int j = 0; j<imgArr.count; j++) {
            NSLog(@"view.height = %f",view.height);
            //            view.height = 228;
            UIImageView *showImg = [[UIImageView alloc] initWithFrame:CGRectMake(40*j+10*(j+1),cell.contentHeight_cons.constant - 50, 40, 40)];
        ...
        CYTuiKuangLeftCell *cell = [tableView dequeueReusableCellWithIdentifier:SellerCellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"CYTuiKuangLeftCell" owner:self options:nil] objectAtIndex:0];
        }
