...
- (void)textChanged:(NSNotification *)notification
{
    ...
    if(self.showWordCountLabel){
        self.wordCountLabel.text = [NSString stringWithFormat:@"%lu/%ld字",(unsigned long)self.text.length,(long)self.maxLength];
...
- (void)updateWordCountLabelFrame
{
    ...
    if(self.text.length > self.maxLength){
        ...
        self.wordCountLabel.textColor = [UIColor getColorWithHexString:@"999999"];
    }
...
- (UILabel *)wordCountLabel
{
    if(!_wordCountLabel){
        ...
        _wordCountLabel.textColor = [UIColor getColorWithHexString:@"999999"];
    }
