...
{
    JKPhotoBrowser  *photoBorwser = [[JKPhotoBrowser alloc] initWithFrame:WINDOW.bounds];
...
- (void)loadAllAssetsForGroups
{
    ...
    NSArray *sort = [assets sortedArrayWithOptions:NSSortStable usingComparator:^(ALAsset *obj1, ALAsset* obj2){
        NSString* filename1 = [[obj1 defaultRepresentation] filename];
        NSString *filename2  =[[obj2 defaultRepresentation] filename];
        NSComparisonResult restult = [filename1 compare:filename2];
        if(restult == NSOrderedAscending) {
            return NSOrderedDescending;
        }else if(restult == NSOrderedDescending) {
    ...
    self.assetsArray = [NSMutableArray arrayWithArray:sort];
...
- (void)startPhotoAssetsViewCell:(JKAssetsViewCell *)assetsCell
{
    if (self.selectedAssetArray.count>=self.maximumNumberOfSelection) {
        NSString  *str = [NSString stringWithFormat:@"最多选择%d张照片",(int)self.maximumNumberOfSelection];
...
- (void)resetFinishFrame
{
    ...
    self.finishLabel.text = [NSString stringWithFormat:@"下一步(%d)",(int)self.selectedAssetArray.count];
