    ...
    if ([correspondingIndexPath subRow] == 0)
    {
        ...
        if (expandableCell.isExpandable)
        {
            ...
            if (isExpanded)
            {
                ...
                expandableCell.backgroundColor = [UIColor grayBackgroundColor];
                
                expandableCell.titleLbl.textColor = [UIColor brownTitleColor];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell respondsToSelector:@selector(isExpandable)])
    {
        ...
        if ([_SKSTableViewDelegate respondsToSelector:@selector(tableView:didSelectRowAtIndexPath:)])
        {
            ...
            if (correspondingIndexPath.subRow == 0 && correspondingIndexPath.row == 0)
            {
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        if ([_SKSTableViewDelegate respondsToSelector:@selector(tableView:willDisplaySubCell:forRowAtIndexPath:)])
        {
            NSIndexPath *correspondingIndexPath = [self correspondingIndexPathForRowAtIndexPath:indexPath];
            
            [_SKSTableViewDelegate tableView:self willDisplaySubCell:cell forRowAtIndexPath:correspondingIndexPath];
        }
