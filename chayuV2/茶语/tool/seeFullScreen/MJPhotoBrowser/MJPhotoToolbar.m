...
//        [SVProgressHUD showSuccessWithStatus:@"成功保存到相册"];
        [Itost showMsg:@"成功保存到相册" inView:nil];
...
- (void)setCurrentPhotoIndex:(NSUInteger)currentPhotoIndex
{
    ...
    NSDictionary * dic = _dataArray[_currentPhotoIndex];
    
    NSString * currentTypeName = [dic objectForKey:@"TypeName"];
    
    // 更新页码
    if(currentTypeName)
    {
        _indexLabel.text = [NSString stringWithFormat:@"%d / %d—%@", _currentPhotoIndex + 1, _photos.count, currentTypeName];
    }
    else
    {
    ...
    MJPhoto *photo = _photos[_currentPhotoIndex];
    // 按钮
    _saveImageBtn.enabled = photo.url != nil && !photo.save;
}
