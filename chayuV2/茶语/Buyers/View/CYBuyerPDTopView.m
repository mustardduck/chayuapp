...
//    [_evaTitleView layoutIfNeeded];
    ...
    CGFloat labHeight = [UICommon lableHeightWithString:_productModel.desc Size:CGSizeMake(self.width - 40,MAXFLOAT) fontSize:14.];

    [UICommon setLabelPadding:_productDesLbl text:_productModel.desc padding:11.];
    NSInteger numberOfLines = labHeight / 14.0;
    _productDesLbl.height = labHeight + 11. * (numberOfLines - 1) + 20;
    
    
    self.endHeight = _goods_details_height.constant + _productDesLbl.height + 55;
...
- (IBAction)like_click:(id)sender {
    ...
    [CYWebClient Post:@"userdoEnjoy" parametes:@{@"goods_id":_productModel.goodsId} success:^(id responObject) {
        ...
        if(self.delegate && [self.delegate respondsToSelector:@selector(PDTopViewlikeClicked)])
        {
            [self.delegate PDTopViewlikeClicked];
        }
