...
- (void)setCouponinfo:(NSDictionary *)couponinfo
{
    ...
    if (_couponinfo.count>0) {
        ...
        {
            [_couponBtn setTitle:_corderModel.couponTitle forState:UIControlStateNormal];
            //            _couponLbl.text = _corderModel.couponTitle;
        }
        else
        {
            //            _couponLbl.text = [_couponinfo objectForKey:@"title"];
            [_couponBtn setTitle:[NSString stringWithFormat:@"抵扣券 - ￥%.2f", [[_couponinfo objectForKey:@"money"] floatValue]] forState:UIControlStateNormal];
...
-(void) setShipPrice:(NSString *)shipPrice
{
    ...
    if (!_shipPrice) {
        ...
        CGFloat shipprice = [_shipPrice floatValue];
        if (shipprice > 0) {
