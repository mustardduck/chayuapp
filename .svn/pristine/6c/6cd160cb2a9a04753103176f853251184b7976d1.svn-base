...
- (void)viewDidLoad {
    ...
    [CYWebClient Post:@"GoodsDetail" parametes:@{@"goods_id":_goodId} success:^(id responObj) {
        ...
        if([productModel.type intValue] == 2)//茗星商品
        {
            ...
            _pdDetailView.seeFullBlock = ^(NSInteger currentPage, NSArray * imgArr)
            {
        ...
        if ([productModel.isSaleBySaleClient integerValue]!=1) {
            NSInteger sale_client = [productModel.sale_client integerValue];
            _nomoreBtn.hidden = NO;
            [_nomoreBtn setBackgroundColor:NAVTITLECOLOR];
            _nomoreBtn.userInteractionEnabled = YES;
            [_nomoreBtn addTarget:self action:@selector(gotowebView:) forControlEvents:UIControlEventTouchUpInside];
            switch (sale_client) {
                case 1://pc端
                {
                    [_nomoreBtn setTitle:@"请前往桌面版购买" forState:UIControlStateNormal];
                    break;
                }
                case 2://触屏版
                {
                    [_nomoreBtn setTitle:@"请前往触屏版购买" forState:UIControlStateNormal];
                    break;
                }
                case 3://手机版
                {
