...
-(CGFloat )tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGSize lableSize = [body boundingRectWithSize:CGSizeMake(SCREEN_WIDTH-40,MAXFLOAT) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
    height -=17;
...
- (IBAction)cancel_click:(id)sender {
    ...
    if (selectArr.count >0) {
        ...
        [CYWebClient basePost:@"2.0_user.notify.delete" parametes:@{@"ids":collStr} success:^(id responObj) {
            NSInteger state = [[responObj objectForKey:@"state"] integerValue];
            if (state == 400) {
                NSString *titleStr = [NSString stringWithFormat:@"删除(0)"];
