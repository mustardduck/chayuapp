...
- (void)viewDidLoad {
    ...
    [dataArr addObject:@{@"ico":@"daohang_chaping",@"title":@"茶评",@"isOpen":@(NO),@"fenlei":chapingArr,@"selectico":@"danghang_chaping_s"}];
    [dataArr addObject:@{@"ico":@"daohang_shiji",@"title":@"市集",@"isOpen":@(NO),@"fenlei":shijiArr,@"selectico":@"danghang_shiji_s"}];
    [dataArr addObject:@{@"ico":@"daohang_quanzi",@"title":@"圈子",@"isOpen":@(NO),@"fenlei":quanziArr,@"selectico":@"danghang_quanzi_s"}];
    [dataArr addObject:@{@"ico":@"daohang_wenzhang",@"title":@"文章",@"isOpen":@(NO),@"fenlei":wenzhangArr,@"selectico":@"danghang_shiji_s"}];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (isOpen) {
        ...
        UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(46,10,225*SCREENBILI,150*SCREENBILI)];
        imgView.image = [UIImage imageNamed:@"750×500"];
        if (indexPath.row< [fenleiArr count]-1) {
...
-(void)bangdan_click:(UIButton *)sender
{
    ...
    if (url.length==0) {
        ...
        vc.typeName = [NSString stringWithFormat:@"%@评分", [info objectForJSONKey:@"name1"]];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    NSString *imgname = isOpen?[info objectForKey:@"selectico"]:[info objectForKey:@"ico"];
    
    UIImage *img = [UIImage imageNamed:imgname];
    header.ico_img.image = img;
    ...
    header.titleLbl.textColor = isOpen?MAIN_COLOR:btnTitle_COLOR;
    //    if (section>0) {
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
