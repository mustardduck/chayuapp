...
- (void)showPhotoViewAtIndex:(NSInteger)index
{
    ...
    if(photo.showedOriginImage || [photo.originUrl isEqual:photo.url] || ( !photo.url && photo.originUrl ) )
    {
        _oriBtn.hidden = YES;
    }
    else
...
- (void)loadImageNearIndex:(NSInteger)index
{
    if (index > 0) {
        ...
        {
            [UICommon downloadWithURL:photo.originUrl];
        }
        else
        {
            [UICommon downloadWithURL:photo.url];
        }
    }
